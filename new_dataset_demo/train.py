...
def create_callback(training_model, prediction_model, validation_generator, args):
    ...
    if args.snapshots:
        ...
        checkpoint = keras.callbacks.ModelCheckpoint(
            os.path.join(
                args.snapshot_path,
                'retinanet_inception_wider_{{epoch:02d}}.h5'
            ),
            verbose=1,
...
def save_model(model, file_name='retinanet_inception_wider.json'):
...
def load_datasets(args):
    train_set      = WiderDataset(args.wider_path, 'train')
    validation_set = WiderDataset(args.wider_path, 'val'  )
...
def check_args(args):
    assert args.wider_path is not None, 'Must have the COCO dataset'
...
def parse_args(args):
    ...
    parser.add_argument('--batch-size',
        ...
        default=1, type=int)
    parser.add_argument('--wider-path',
        help='Path to dataset directory (ie. /tmp/wider)',
        type=str)
